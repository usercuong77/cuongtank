...
const appNsPath = path.join(root, 'assets/js/runtime/app-namespace.js');
...
if (fs.existsSync(cssPath)) {
  ...
  for (const ref of cssRefs) {
    const resolved = path.posix.join('assets/css', ref).replace(/\\/g, '/');
    const finalRef = ref.startsWith('/') ? ref.slice(1) : resolved;
    if (!existsLocal(finalRef)) missing.push(finalRef);
  }
}
